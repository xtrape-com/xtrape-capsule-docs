...
================================================================================
原始文件 / Original File: 01-embedded-agent.md
...
> **Precedence rule**: When this document and `08-decisions/` ADRs or `09-contracts/openapi/opstage-ce-v0.1.yaml` disagree, the ADRs and OpenAPI contract win for CE（社区版） v0.1. In particular: CommandResult uses `success / message / data / error` (not `outputText / errorMessage / resultJson`), and `dangerLevel` is `LOW | MEDIUM | HIGH` (no `CRITICAL`).

This document 定义 the **Embedded Agent（代理）** model for the `xtrape-capsule` product family.
...
The purpose of an Embedded Agent（代理） is to make a Capsule Service（胶囊服务） governable by Opstage（运维舞台） without requiring a separate Agent（代理） process.

The Embedded Agent（代理） allows a Capsule Service（胶囊服务） to:

- register itself with Opstage（运维舞台） through an Agent（代理） identity;
- authenticate to Opstage（运维舞台） Backend;
- send heartbeat;
...
The Embedded Agent（代理） is the simplest Agent（代理） mode and the best fit for CE（社区版） v0.1.

---

## 2. Positioning

An Embedded Agent（代理） is:

> An in-process governance bridge between a Capsule Service（胶囊服务） and Opstage（运维舞台） Backend.
...
The Capsule Service（胶囊服务） remains the business runtime. The Embedded Agent（代理） only exposes governance metadata and safe predefined operations.

---

## 3. CE（社区版） Scope

CE（社区版） v0.1 should implement one Embedded Agent（代理） SDK:
...
- load stored Agent（代理） token;
- register with registration token when needed;
- store issued Agent（代理） token locally;
- send heartbeat;
- report service manifest;
...
CE（社区版） v0.1 Embedded Agent（代理） should not implement:

- Java Embedded Agent（代理） SDK;
- Python Embedded Agent（代理） SDK;
- Go Embedded Agent（代理） SDK;
- sidecar Agent（代理）;
- external Agent（代理）;
- host Agent（代理）;
- Kubernetes Agent（代理）;
...
The Embedded Agent（代理） runs inside the Capsule Service（胶囊服务） process.
...
The Embedded Agent（代理） should be lightweight and should not require a separate runtime service.

---

## 6. Process Isolation Principle

Because the Embedded Agent（代理） runs in-process, it must avoid destabilizing the Capsule Service（胶囊服务）.

Rules:

- Agent（代理） startup failure should not crash the business service by default;
...
- Agent（代理） should provide optional strict mode if a service wants governance to be mandatory.

Default CE（社区版） behavior should favor business runtime continuity.

---

## 7. Developer Experience Target

A Capsule Service（胶囊服务） developer should integrate the Embedded Agent（代理） with a small amount of code.
...
The Embedded Agent（代理） should support configuration from code and environment variables.
...
The Embedded Agent（代理） reports one Capsule Service（胶囊服务） identity in CE（社区版） v0.1.
...
During startup, the Embedded Agent（代理） should:

1. validate local configuration;
2. load stored Agent（代理） token if available;
...
- do not crash the Capsule Service（胶囊服务） by default.

If registration token is missing and no stored Agent（代理） token exists:
...
On process shutdown, the Embedded Agent（代理） should:
...
CE（社区版） v0.1 does not need a formal Agent（代理） deregistration call.
...
Registration request body (matches OpenAPI `RegisterAgentRequest`):

```json
{
  "registrationToken": "opstage_reg_...",
  "agent": {
    "code": "local-node-agent",
    "name": "Local Node Agent",
    "mode": "embedded",
    "runtime": "nodejs",
    "version": "0.1.0"
  },
...
Registration response body (matches OpenAPI `RegisterAgentResponse`):
...
Raw Agent（代理） token is returned only once and must be stored locally by the Agent（代理） SDK.

Raw Agent（代理） token should only be returned once.

---

## 14. Token Store

The Embedded Agent（代理） needs a local token store.

CE（社区版） v0.1 recommended token store:
...
Future EE（企业版） may support OS keychain, Kubernetes Secret, Vault reference, or managed token stores.

---

## 15. Agent（代理） Token Usage

After registration, the Embedded Agent（代理） uses Agent（代理） token for Agent（代理） API calls.
...
The Embedded Agent（代理） reports a manifest describing the Capsule Service（胶囊服务）.
...
CE（社区版） v0.1 can report manifest at startup and after reconnect.
...
- config editing is future EE（企业版） work.
...
- poll only after Agent（代理） token is available;
...
For each `ACTION` Command, the Embedded Agent（代理） should:
...
CE（社区版） v0.1 should execute one Command at a time unless concurrency is explicitly designed.
...
The Embedded Agent（代理） should retry transient failures.
...
CE（社区版） implementation may use a simpler backoff but should avoid tight failure loops.

---

## 24. Offline Behavior

If Opstage（运维舞台） Backend is offline, the Embedded Agent（代理） should:
...
Embedded Agent（代理） logs should include:
...
Embedded Agent（代理） logs must not include:
...
The Embedded Agent（代理） must not send raw secrets by default.
...
The CE（社区版） demo service should include safe demo actions.
...
The Embedded Agent（代理） SDK must not include built-in unsafe actions such as:
...
CE（社区版） should not encourage remote shell-style usage.

---

## 30. 测试 Requirements

Embedded Agent（代理） SDK should be tested for:
...
The Embedded Agent（代理） should reserve clean extension points:
...
These fields support future EE（企业版） and Cloud（云版） evolution.

Do not implement future systems in CE（社区版） just because fields exist.

---

## 32. Future EE（企业版） Direction
...
- action-level RBAC;
- sidecar/external Agent（代理） alternatives.

These are not CE（社区版） v0.1 requirements.

---

## 33. Future Cloud（云版） Direction
...
- managed alerts on Agent（代理） state.
...
The CE（社区版） Embedded Agent（代理） is acceptable when:

- SDK can be installed in a Node.js demo Capsule Service（胶囊服务）;
- SDK can register with Backend using registration token;
- SDK stores issued Agent（代理） token locally;
- SDK reuses stored Agent（代理） token after restart;
...
> Keep the Embedded Agent（代理） small, safe, in-process, outbound-first, and limited to explicit governance capabilities rather than unrestricted runtime control.
