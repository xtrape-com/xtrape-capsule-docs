...
================================================================================
原始文件 / Original File: 04-node-agent-sdk.md
...
- Edition: CE（社区版）
- Priority: 高
- Audience: agent SDK developers, Capsule Service（胶囊服务） developers, backend developers, test engineers, AI coding agents
...
The purpose of the Node.js Agent（代理） SDK is to provide a lightweight integration library for Node.js Capsule Services.

The SDK should make it easy to:

- configure Opstage（运维舞台） Backend connection;
- register with a registration token;
- store and reuse Agent（代理） token;
- send heartbeat;
...
The SDK is the main developer-facing integration point for CE（社区版） Capsule Services.
...
because the SDK belongs to the `xtrape-capsule` domain and implements the Node.js Agent（代理） capability.
...
This example should be used as the reference for the CE（社区版） demo Capsule Service（胶囊服务）.
...
## 10. Agent（代理） Registration

The SDK should register when no valid stored Agent（代理） token exists.
...
If no stored Agent（代理） token and no registration token exists, SDK should:
...
## 12. Agent（代理） Token Handling

Agent（代理） token rules:

- use `Authorization: Bearer <agentToken>` for Agent（代理） API calls;
- never log raw token;
...
Token rotation is future EE（企业版） work and not required in SDK v0.1.
...
2. load or register Agent（代理） token;
...
CE（社区版） v0.1 does not require Backend deregistration on stop.
...
- run after Agent（代理） token is available;
- send heartbeat periodically;
- include Agent（代理） runtime metadata;
...
Heartbeat endpoint should match Backend Agent（代理） API spec.
...
- process Commands assigned to this Agent（代理）;
...
CE（社区版） implementation can start simple, but must avoid tight retry loops.
...
### 28.3 Agent（代理） token invalid or revoked
...
- set `Authorization` header for Agent（代理） API calls;
...
The SDK should be usable from the CE（社区版） demo Capsule Service（胶囊服务） without local hacks.

---

## 34. Demo Capsule Service（胶囊服务） Requirements

The repository should include or reference a demo Capsule Service（胶囊服务） using the SDK.
...
SDK should expose its version in Agent（代理） metadata.
...
Do not implement EE（企业版）/Cloud（云版） capabilities unless needed by CE（社区版） v0.1.

---

## 38. Future EE（企业版） Direction

Future EE（企业版） may extend the Node SDK with:
...
- sidecar/external Agent（代理） interoperability helpers.

These are not SDK v0.1 requirements.

---

## 39. Future Cloud（云版） Direction

Future Cloud（云版） may extend SDK usage with:
...
Node Agent（代理） SDK v0.1 is acceptable when:
...
- SDK stores Agent（代理） token locally;
- SDK reuses Agent（代理） token after restart;
...
- SDK handles invalid Agent（代理） token safely;
...
The Node.js Agent（代理） SDK is the CE（社区版） reference Agent（代理） implementation.

It should be small, safe, easy to integrate, and sufficient to prove the full Opstage（运维舞台） governance loop with a real Node.js Capsule Service（胶囊服务）.

The most important Node Agent（代理） SDK rule is:
