...
================================================================================
原始文件 / Original File: 01-opstage-ui.md
...
This document 定义 the UI subsystem of **Opstage（运维舞台）**.

Opstage（运维舞台） UI is the human-facing console for operating Capsule Services through Opstage（运维舞台） Backend and Agent（代理）-based governance.

The current implementation focus is **Opstage（运维舞台） CE（社区版）**. EE（企业版） and Cloud（云版） UI capabilities are future planning tracks and must not expand the CE（社区版） v0.1 implementation scope.

---

## 1. Purpose

The purpose of Opstage（运维舞台） UI is to make the Capsule governance loop visible and operable.
...
The UI should make lightweight services governable without forcing every Capsule Service（胶囊服务） to build its own admin console.
...
> The operator console for Capsule Service（胶囊服务） governance.
...
- a billing portal in CE（社区版）.

The first UI identity should remain runtime governance for Capsule Services.

---

## 3. Relationship with Opstage（运维舞台） Backend

Opstage（运维舞台） UI must communicate only with Opstage（运维舞台） Backend through Admin APIs.
...
Opstage（运维舞台） UI should follow these principles:

1. Make runtime status obvious.
2. Distinguish Agent（代理） status, service status, health status, and freshness.
3. Prefer simple tables and detail pages over complex dashboards in CE（社区版）.
4. Expose only predefined actions.
5. Never provide arbitrary shell execution in CE（社区版）.
...
10. Avoid disabled EE（企业版）/Cloud（云版）-only navigation clutter in CE（社区版）.
...
Opstage（运维舞台） CE（社区版） UI should implement the minimum complete governance UI.

Required CE（社区版） pages:
...
## 6. CE（社区版） UI Non-Goals

CE（社区版） v0.1 UI should not implement:
...
CE（社区版） has one implicit default Workspace, so a workspace switcher is not required.
...
CE（社区版） navigation should be simple:
...
The Login page authenticates local CE（社区版） users.
...
CE（社区版） Dashboard should prioritize clarity over decorative charts.
...
but CE（社区版） v0.1 may defer them unless supported by Backend.

---

## 13. Agent（代理） Detail Page

Agent（代理） Detail should show Agent（代理） identity, connectivity, and reported services.
...
## 15. Capsule Service（胶囊服务） Detail Page

Capsule Service（胶囊服务） Detail is the most important CE（社区版） UI page.
...
CE（社区版） v0.1 may use simple JSON input instead of a full dynamic form engine.
...
The Commands page 提供 operation history.
...
The Audit Logs page 提供 traceability.
...
System Settings should show CE（社区版） runtime information and basic operational settings.
...
For Capsule Services, show **Effective 状态** as the primary status.
...
CE（社区版） UI must only execute predefined actions.

CE（社区版） UI must not provide:
...
CE（社区版） may use simple polling or manual refresh.

Recommended behavior:

- Dashboard: manual refresh or light polling;
- Agent（代理） list: manual refresh or light polling;
- Service detail: manual refresh or light polling;
- Command detail: poll while command is pending or dispatched.

Do not require WebSocket in CE（社区版） v0.1.

---

## 24. Mobile Compatibility

CE（社区版） UI should be usable on mobile browsers for viewing and simple operations.
...
## 25. EE（企业版） UI Extension Direction

Future EE（企业版） UI may add:
...
- sidecar/external Agent（代理） management;
...
## 26. Cloud（云版） UI Extension Direction

Future Cloud（云版） UI may add:
...
Opstage（运维舞台） CE（社区版） UI is acceptable when:

- user can log in;
- dashboard loads;
- user can create a registration token;
- Agent（代理） appears after registration;
- Agent（代理） online/offline status is visible;
- Capsule Service（胶囊服务） appears after service report;
- service detail shows 概述, Manifest, Health, Configs, Actions, Commands, and Audit;
...
- UI does not show disabled EE（企业版）/Cloud（云版）-only pages in CE（社区版）.

---

## 28. Summary

Opstage（运维舞台） UI should make Capsule Services understandable and operable through a simple governance console.

The most important UI rule is:

> Make service governability visible, expose only safe predefined operations, and keep CE（社区版） UI focused on the core governance loop.
