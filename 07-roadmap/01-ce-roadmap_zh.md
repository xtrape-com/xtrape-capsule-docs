...
The first CE（社区版） milestone is **CE（社区版） v0.1**. CE（社区版） v0.1 should prove the complete Capsule governance loop with
Opstage（运维舞台） Backend, Opstage（运维舞台） UI, SQLite, and the Node.js Embedded Agent（代理） SDK.
