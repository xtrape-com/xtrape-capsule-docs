...
## 12. CE v0.4 方向 — 轻量级服务通信元数据

如果治理内核稳定，CE v0.4 可能引入轻量级服务通信元数据。

目标不是实现完整的服务治理。目标是标准化 capsule 管理的服务如何描述其内部端点和能力。
...
## 13. CE v0.5 方向 — Capsule Bus 规划和早期协调

如果 CE v0.4 证明了服务元数据模型，CE v0.5 可能引入 Capsule Bus 的早期规划和有限原型。
