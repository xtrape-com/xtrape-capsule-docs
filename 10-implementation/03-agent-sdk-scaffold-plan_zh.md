...
================================================================================
原始文件 / Original File: 03-agent-sdk-scaffold-plan.md
...
# CE（社区版） v0.1 Node Agent（代理） SDK Scaffold Plan
...
- never log raw token;
- if Agent（代理） token is rejected, surface a clear re-registration error.
...
- avoid crashing the Capsule Service（胶囊服务） by default;
...
- stores Agent（代理） token;
- reuses stored Agent（代理） token;
- sends heartbeat;
