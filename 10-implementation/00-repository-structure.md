...
> **Precedence rule**: When this document and `08-decisions/` ADRs disagree, the ADRs win for CE v0.1. The authoritative naming and repository decisions live in [ADR 0008](../08-decisions/0008-naming-and-repositories.md) and [ADR 0009](../08-decisions/0009-contracts-spec-and-bindings.md).
...
This is the only repository where pnpm workspace is meaningful. It contains the CE backend, UI, demo Capsule Service, deployment assets, and CE-internal helper packages.
...
There is **no** `packages/contracts/` and **no** `packages/agent-node/` in the CE monorepo. Both come from npm:
...
It must register with Opstage, report manifest/health/config/actions, and expose demo actions:
...
Small shared utilities (CE-internal):
...
Recommended root scripts for the CE monorepo:
...
## 6. Local Development Across Repos
...
- `pnpm verify` from the CE repo root runs `lint + typecheck + test + build` for all CE workspace packages, green.
