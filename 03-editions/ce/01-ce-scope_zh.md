...
================================================================================
原始文件 / Original File: 01-ce-scope.md
...
Opstage（运维舞台） CE（社区版） v0.1 must focus on one goal:

> Make a Node.js Capsule Service（胶囊服务） visible, manageable, and auditable through Opstage（运维舞台） using an embedded Agent（代理） SDK.
...
CE（社区版） v0.1 should be useful, lightweight, self-hosted, and simple to deploy.

---

## 2. In Scope

The following capabilities are in scope for CE（社区版） v0.1.

### 2.1 Local admin access

CE（社区版） v0.1 must provide a simple local admin login.
...
CE（社区版） v0.1 must create and use one default Workspace.
...
### 2.3 Agent（代理） registration

CE（社区版） v0.1 must support Agent（代理） registration.
...
- token rotation UI;
- device attestation;
- advanced Agent（代理） permission policies;
- sidecar Agent（代理）;
- external Agent（代理）.

### 2.4 Agent（代理） heartbeat

CE（社区版） v0.1 must support Agent（代理） heartbeat.

Required:

- heartbeat API;
- last heartbeat timestamp;
- Agent（代理） online/offline calculation;
- heartbeat timeout configuration;
- UI display of Agent（代理） freshness.
...
### 2.5 Capsule Service（胶囊服务） reporting

CE（社区版） v0.1 must support Capsule Service（胶囊服务） metadata reporting through the Agent（代理）.
...
- service-to-Agent（代理） association;
...
CE（社区版） v0.1 must support latest health visibility.

Required:

- health provider support in Node.js embedded Agent（代理） SDK;
...
CE（社区版） v0.1 must support configuration metadata visibility.
...
CE（社区版） v0.1 must support predefined actions.

Required:

- ActionDefinition shape;
- action definitions reported by Agent（代理）;
- UI action list;
- action request API;
...
CE（社区版） v0.1 must support Commands for action execution.

Required:

- `ACTION` command type;
- Command creation from UI action request;
- Command polling API for Agent（代理）;
...
CE（社区版） v0.1 must support basic audit logs.
...
- first Capsule Service（胶囊服务） report;
- command creation;
- command success;
- command failure;
...
CE（社区版） v0.1 must provide a Web UI.
...
Minimum Capsule Service（胶囊服务） detail tabs:
...
### 2.12 Node.js embedded Agent（代理） SDK

CE（社区版） v0.1 must ship a Node.js embedded Agent（代理） SDK.

Required SDK capabilities:

- registration;
- Agent（代理） token persistence hook or guidance;
- heartbeat;
- manifest reporting;
- health provider;
- config provider;
...
The SDK must not block the Capsule Service（胶囊服务） from starting if Opstage（运维舞台） is unavailable.

### 2.13 Demo Capsule Service（胶囊服务）
...
CE（社区版） v0.1 must support SQLite as the default database.
...
CE（社区版） v0.1 must support a simple Docker-based local deployment.
...
The following items are out of scope for CE（社区版） v0.1.
...
### 4.3 高 availability and cluster deployment
...
Agent（代理） SDK is not in scope for:
...
CE（社区版） v0.1 should implement or represent at least:
...
Objects that should not be implemented as first-class CE（社区版） v0.1 objects unless necessary:
...
CE（社区版） v0.1 should include APIs in these groups.
...
## 8. Required UI 状态 Behavior

CE（社区版） UI must not display stale services as confidently online.
...
CE（社区版） v0.1 scope is complete when the following scenario works end to end.

### 10.1 Startup

- Opstage（运维舞台） Backend starts with SQLite.
- Opstage（运维舞台） UI is accessible.
- Default Workspace exists.
- Admin user can log in.

### 10.2 Agent（代理） registration

- A registration token can be created or bootstrapped.
- Demo Capsule Service（胶囊服务） starts with Node.js embedded Agent（代理） SDK.
- Agent（代理） registers successfully.
- Backend issues Agent（代理） token.
- UI shows the Agent（代理）.

### 10.3 Service governance

- Agent（代理） reports demo Capsule Service（胶囊服务） manifest.
- UI shows the Capsule Service（胶囊服务）.
...
- Agent（代理） polls the Command.
- Agent（代理） executes the local handler.
...
- When demo Agent（代理） stops, Backend eventually marks Agent（代理） as offline.
...
CE（社区版） v0.1 scope is intentionally narrow.
