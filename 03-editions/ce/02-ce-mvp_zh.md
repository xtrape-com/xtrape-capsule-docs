...
================================================================================
原始文件 / Original File: 02-ce-mvp.md
...
This document 定义 the **Minimum Viable Product** for **Opstage（运维舞台） CE（社区版） v0.1**.
...
The goal of CE（社区版） v0.1 MVP is:

> A developer can run Opstage（运维舞台） CE（社区版） locally, start a demo Node.js Capsule Service（胶囊服务） with an embedded Agent（代理） SDK, see it in the UI, trigger a predefined action, see the command result, and inspect the audit log.
...
CE（社区版） v0.1 should deliver this promise:

> In a few minutes, a developer can self-host Opstage（运维舞台） CE（社区版） and bring a Node.js Capsule Service（胶囊服务） under basic governance.
...
### 3.1 Start Opstage（运维舞台） CE（社区版）

As a developer, I want to start Opstage（运维舞台） CE（社区版） locally so that I can manage Capsule Services without external SaaS dependencies.
...
### 3.2 Register an Agent（代理）

As a developer, I want a demo Capsule Service（胶囊服务） to register through a Node.js embedded Agent（代理） so that it appears in Opstage（运维舞台）.

Acceptance:

- Registration token exists.
- Agent（代理） registers with Backend.
- Backend issues Agent（代理） token.
- Agent（代理） token is stored as hash in Backend.
- UI shows the Agent（代理）.

### 3.3 Report a Capsule Service（胶囊服务）

As a developer, I want the Agent（代理） to report a Capsule Service（胶囊服务） manifest so that Opstage（运维舞台） can display the service.
...
As an operator, I want to see Agent（代理） and service status so that I know whether the service is currently governable.

Acceptance:

- Agent（代理） heartbeat updates `lastHeartbeatAt`.
- UI shows Agent（代理） online/offline status.
- Agent（代理） reports service health.
- UI shows HealthStatus.
- Backend calculates effective Capsule Service（胶囊服务） status.
- Stale status is shown when Agent（代理） stops heartbeating.
...
- Agent（代理） executes a registered action handler.
...
- Login is audited.
- Agent（代理） registration is audited.
...
### 4.3 Agent（代理） registration

MVP must include:

- registration token generation or bootstrap;
- Agent（代理） registration endpoint;
- Agent（代理） token issuance;
- token hash storage;
- Agent（代理） status model;
- Agent（代理） list UI;
- Agent（代理） detail UI.
...
### 4.4 Agent（代理） heartbeat

MVP must include:

- heartbeat endpoint;
- heartbeat interval returned to Agent（代理）;
...
### 4.5 Capsule Service（胶囊服务） report
...
- health provider in Agent（代理） SDK;
- health report from Agent（代理）;
...
- action list UI;
- action request API;
- predefined action handler execution through Agent（代理）.
...
- MySQL/PostgreSQL;
- sidecar/external Agent（代理）;
- EE（企业版） and Cloud（云版） editions;
- richer UI;
- stronger security.
...
### 6.6 Capsule Service（胶囊服务） Detail
...
The Node.js embedded Agent（代理） SDK should provide a minimal developer-facing API.
...
## 9. MVP Demo Capsule Service（胶囊服务）
...
### Slice 5: Agent（代理） SDK and demo service
...
1. Start demo Capsule Service（胶囊服务） with registration token.
2. Confirm Agent（代理） appears in UI.
3. Confirm Agent（代理） status is `ONLINE`.
4. Confirm Capsule Service（胶囊服务） appears in UI.

### 13.3 Inspect service

1. Open Capsule Service（胶囊服务） detail page.
...
3. Confirm Agent（代理） executes Command.
...
3. Confirm Agent（代理） becomes `OFFLINE`.
4. Confirm Capsule Service（胶囊服务） becomes `STALE`.
...
CE（社区版） v0.1 MVP is complete when:

- the end-to-end demo works;
- README can guide a new developer to run the system;
- demo service proves Agent（代理） SDK integration;
