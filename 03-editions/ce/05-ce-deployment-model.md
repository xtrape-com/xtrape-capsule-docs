...
The goal of CE deployment is:

> A developer or small team can run Opstage CE locally or on a small private server with minimal operational overhead.
...
CE should also support local development with separate Backend, UI, and demo service processes.

---

## 2. Deployment Principles

CE v0.1 deployment should follow these principles:

1. Keep the first deployment simple.
2. Use SQLite by default.
3. Use a persistent data volume.
4. Expose only one HTTP port for Opstage CE by default.
5. Do not require Kubernetes.
6. Do not require Redis, RabbitMQ, Kafka, or other queues.
7. Do not require MySQL/PostgreSQL in CE v0.1.
8. Do not require external SaaS services.
9. Keep the Agent connection outbound-first.
10. Preserve future paths for EE and Cloud deployment models.

---

## 3. Supported Deployment Modes

CE v0.1 should support three deployment modes.
...
Primary user-facing deployment mode for CE v0.1.
...
This mode is recommended for most CE users.

### 3.3 Docker Compose mode

Optional convenience mode.

Useful when running Opstage CE together with a demo Capsule Service.
...
The demo Capsule Service may not need to expose a public port if it uses only embedded Agent communication.
...
Single-container mode is the preferred CE user deployment.
...
CE v0.1 may run as one Node.js process serving both Admin APIs, Agent APIs, and UI static assets.
...
This is acceptable for CE v0.1.

Future EE may split UI and Backend.
...
CE v0.1 should avoid storing large logs or large binary files in the SQLite database.

---

## 8. Database Migration

CE v0.1 should provide a simple migration mechanism.
...
The first CE version may run migrations on startup if it is reliable and clearly documented.
...
CE v0.1 needs a simple first-run setup.
...
CE v0.1 may support one of these approaches:
...
Recommended demo Capsule Service environment variables:
...
Agents connect outbound to Opstage Backend.

```text
Agent -> Opstage Backend
```

CE v0.1 should not require Opstage Backend to connect inbound to Agents.
...
Users may put Opstage CE behind a reverse proxy such as Nginx, Caddy, or Traefik.

This is optional and not required for CE v0.1 quick start.

---

## 12. HTTPS

CE v0.1 local development may use HTTP.

For production-like self-hosted usage, users should run Opstage CE behind HTTPS using a reverse proxy.

CE v0.1 does not need to implement built-in certificate management.

Future EE/Cloud may provide stronger TLS and certificate management guidance.

---

## 13. Upgrade Model

CE v0.1 should keep upgrade simple.
...
Use semantic versioning for CE images:
...
CE v0.1 does not need a built-in backup UI.

---

## 15. Observability for Deployment

CE v0.1 should provide simple operational visibility.

Required:

- application logs to stdout/stderr;
- health endpoint for Opstage Backend if practical;
- clear startup logs;
...
CE v0.1 does not need Prometheus metrics or centralized log collection.

---

## 16. Resource Requirements

CE v0.1 should run comfortably on small servers.
...
CE v0.1 deployment should follow these rules:
...
- allow Agent APIs only with valid Agent token.

---

## 18. Out of Scope for CE v0.1 Deployment
...
Future EE/Cloud may add these capabilities.

---

## 19. Future Deployment Paths

CE deployment should preserve future upgrade paths.

| Area | CE v0.1 | Future EE / Cloud |
|---|---|---|
| Process | single process | split UI, Backend, workers |
| Database | SQLite | MySQL, PostgreSQL, managed DB |
| Deployment | single container | Docker Compose, Helm, cluster |
...
## 20. Deployment Acceptance Criteria

CE v0.1 deployment is acceptable when:

- Opstage CE can run locally in development mode;
- Opstage CE can run as a single Docker container;
...
- demo Capsule Service can connect to Backend;
- Agent heartbeat and command polling work across container network;
- logs are visible through container logs;
- documentation provides quick-start commands.

---

## 21. Summary

Opstage CE deployment should optimize for simplicity and trust.
...
> CE should be easy to run before it becomes powerful to operate at scale.
