...
================================================================================
原始文件 / Original File: 08-ce-ui-design.md
...
This document 定义 the UI design for **Opstage（运维舞台） CE（社区版） v0.1**.

The CE（社区版） UI should be a lightweight Web console that allows operators to see, understand, and operate Capsule Services through Opstage（运维舞台） Backend.

---

## 1. UI 设计 Goal

The goal of the CE（社区版） UI is to make the Capsule governance loop visible and operable:
...
CE（社区版） v0.1 UI should follow these principles:

1. Be simple enough for first-time users.
2. Make status and freshness visible.
3. Distinguish Agent（代理） status, Service status, and Health status.
...
9. Avoid EE（企业版）/Cloud（云版）-only features in CE（社区版） v0.1.
...
CE（社区版） v0.1 UI stack (decided by [ADR 0007](../../08-decisions/0007-ui-state-and-data-fetching.md)):
...
Ant 设计 Vue 提供 the same mature admin-console components as the React Ant 设计 ecosystem.

The UI must remain a Web console. A native mobile app is not required for CE（社区版） v0.1.

---

## 4. Information 架构

The CE（社区版） UI should contain these primary sections:
...
CE（社区版） v0.1 should be mobile-readable, not a full mobile application.
...
CE（社区版） v0.1 should implement at least these pages:
...
CE（社区版） v0.1 does not need charts or monitoring panels.
...
## 10. Agent（代理） Detail Page

### 10.1 Purpose

The Agent（代理） Detail page shows identity, connectivity, and reported services for one Agent（代理）.
...
## 12. Capsule Service（胶囊服务） Detail Page

### 12.1 Purpose

The Capsule Service（胶囊服务） Detail page is the most important CE（社区版） UI page.

It should show everything Opstage（运维舞台） knows about a service.
...
CE（社区版） v0.1 does not need a full dynamic form engine. Simple JSON payload input is acceptable if schema rendering is not ready.
...
The Commands page 提供 an operation history view.
...
The Audit Logs page 提供 traceability for important operations.
...
System Settings shows CE（社区版） runtime information and basic settings.
...
CE（社区版） v0.1 should provide a way to create registration tokens.
...
状态 values are not colors, but UI may use consistent tags.
...
CE（社区版） v0.1 may use simple polling or manual refresh.
...
Do not introduce WebSocket dependency in CE（社区版） v0.1.

---

## 21. Mobile Compatibility

CE（社区版） v0.1 should be usable on mobile browsers for viewing.
...
CE（社区版） v0.1 should follow basic usability practices:
...
CE（社区版） v0.1 UI should not include:
...
The CE（社区版） UI is acceptable when:
...
- stopping Agent（代理） eventually makes service stale;
...
The CE（社区版） UI should make lightweight service governance understandable.

The most important UI rule is:

> Make the current governability of each Capsule Service（胶囊服务） obvious, and allow only safe predefined operations.
