...
The purpose of the Node.js Runtime is to define how a Node.js service becomes a Capsule Service governed by Opstage.
...
CE v0.1 Node.js Runtime includes:
...
CE v0.1 Node.js Runtime does not include:
...
- HTTP communication with Opstage Backend;
- safe operation when Opstage Backend is temporarily unavailable.

The Node.js runtime should avoid heavy dependencies in CE v0.1.

---

## 4. SDK Package

Node.js Runtime should use the CE Node Agent SDK package:
...
The SDK is the official CE integration path for Node.js Capsule Services.

---

## 5. Node.js Capsule Service Definition

A Node.js Capsule Service is a Node.js application that:

- owns its business logic;
- integrates the Node Agent SDK;
...
- allows the Agent SDK to communicate with Opstage Backend.

A Capsule Service is not required to expose its own admin UI.

Opstage UI becomes the common governance surface.
...
This example should be the baseline for the CE demo service.

---

## 7. Service Identity

Node.js Capsule Service identity should be stable.
...
- `version` should reflect the Capsule Service version;
...
Registration token and Agent token must not be printed in logs.

---

## 9. Agent Token Store

Node.js runtime should use a file-based token store in CE v0.1.
...
- dependency health is optional in CE.
...
The CE Node.js demo service should include at least two safe actions.
...
Node.js runtime should tolerate Opstage Backend downtime.
...
If no Agent token and no registration token are available:
...
Node.js runtime should stop Agent loops cleanly.
...
CE does not require deregistration on shutdown.
...
Node.js Capsule Service should be easy to run in Docker.
...
so the Agent token survives container restart.
...
A CE Docker Compose demo may include:
...
4. Agent appears in UI;
5. Capsule Service appears in UI;
6. health is visible;
...
Runtime tests may reuse Node Agent SDK tests where appropriate.
...
Future EE may extend Node.js runtime with:
...
- sidecar/external Agent interoperability.
...
Future Cloud may extend Node.js runtime with:
...
### 28.1 Service depends on Opstage to start business logic
...
- Node.js demo Capsule Service can import `@xtrape/capsule-agent-node`;
...
Node.js Runtime is the CE reference runtime for Capsule Services.

It should prove that a lightweight Node.js service can become visible, healthy, configurable by metadata, operable through predefined actions, and auditable through Opstage.
