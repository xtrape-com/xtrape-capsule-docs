...
================================================================================
原始文件 / Original File: 02-java-runtime-planning.md
...
This document 定义 the planned **Java Runtime** integration for the `xtrape-capsule` product family.

Java Runtime is a future extension. It is not a CE（社区版） v0.1 implementation requirement.

The current CE（社区版） implementation focus is **Node.js**. Java Runtime planning exists to keep the Capsule governance model compatible with enterprise Java and Spring-based services in future EE（企业版） or later CE（社区版） versions.

---

## 1. Purpose

The purpose of Java Runtime planning is to define how Java services may become Capsule Services governed by Opstage（运维舞台）.

Java Runtime should eventually define:

- Java service identity conventions;
- Java Agent（代理） SDK package structure;
...
CE（社区版） v0.1 implements Node.js first.

Java Runtime is not required for CE（社区版） v0.1.

CE（社区版） may reserve shared concepts that make Java integration easier later:
...
But CE（社区版） should not implement Java SDK, Maven artifacts, or Spring Boot starter in v0.1.
...
because it 提供 mature lifecycle hooks, configuration conventions, actuator health concepts, and enterprise adoption.
...
||Artifact|Purpose||
|---|---|
||`xtrape-capsule-agent-java`|core Java Agent（代理） SDK||
...
## 6. Java Embedded Agent（代理） Model

The first Java Runtime model should be Embedded Agent（代理）.
...
The Java Agent（代理） should run inside the service process but should avoid destabilizing the business runtime.
...
This is planning only and not a CE（社区版） v0.1 implementation target.
...
Java Capsule Service（胶囊服务） identity should be stable.
...
Java Runtime should tolerate Opstage（运维舞台） Backend downtime.

If Backend is unavailable, Java Agent（代理） should:
...
- Agent（代理） registers with Opstage（运维舞台）;
...
- AuditEvent appears in Opstage（运维舞台）;
- Backend downtime does not crash the service by default.

This demo is future work, not CE（社区版） v0.1.
...
- Agent（代理） token invalid handling;
...
## 22. Future EE（企业版） Direction

Future EE（企业版） may include Java Runtime as an enterprise feature because many enterprise systems are Java-based.

Possible EE（企业版） capabilities:

- official Java Agent（代理） SDK;
- Spring Boot starter;
- Spring Cloud（云版） integration examples;
- Actuator health bridge;
...
## 23. Future Cloud（云版） Direction

Future Cloud（云版） may include Java Runtime support through:

- Cloud（云版） enrollment snippets;
- workspace-scoped registration token examples;
- Agent（代理） Gateway configuration;
...
### 24.1 Java SDK before Node.js CE（社区版） is stable
...
- Java/Spring Boot service can include the Java Agent（代理） dependency;
