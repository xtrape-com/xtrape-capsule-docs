...
================================================================================
原始文件 / Original File: 03-python-runtime-planning.md
...
This document 定义 the planned **Python Runtime** integration for the `xtrape-capsule` product family.
...
The purpose of Python Runtime planning is to define how Python services or workers may become Capsule Services governed by Opstage（运维舞台）.

Python Runtime should eventually define:

- Python service identity conventions;
- Python Agent（代理） SDK package structure;
...
Python Runtime is not required for CE（社区版） v0.1.

CE（社区版） may reserve shared concepts that make Python integration easier later:
...
But CE（社区版） should not implement Python SDK, PyPI package publishing, or FastAPI helper in v0.1.
...
## 6. Python Embedded Agent（代理） Model

The first Python Runtime model should be Embedded Agent（代理）.
...
The Python Agent（代理） should run inside the service process but should avoid destabilizing the business or automation runtime.
...
A FastAPI helper may integrate Agent（代理） lifecycle with application startup and shutdown.
...
- start Agent（代理） on FastAPI startup;
- stop Agent（代理） on shutdown;
...
Registration token and Agent（代理） token must not be logged.

---

## 11. Service Identity

Python Capsule Service（胶囊服务） identity should be stable.
...
- raw credentials and browser sessions must not be sent to Opstage（运维舞台）;
...
Python Runtime should tolerate Opstage（运维舞台） Backend downtime.

If Backend is unavailable, Python Agent（代理） should:
...
- do not send raw browser session data to Opstage（运维舞台）;
- use `secretRef` for sensitive references;
...
## 25. Future EE（企业版） Direction

Future EE（企业版） may include Python Runtime because Python is highly relevant for AI automation and data-processing services.

Possible EE（企业版） capabilities:

- official Python Agent（代理） SDK;
- FastAPI helper;
...
## 26. Future Cloud（云版） Direction

Future Cloud（云版） may include Python Runtime support through:
...
### 27.1 Python SDK before Node.js CE（社区版） is stable
...
### 27.4 Browser/session data leaks into Opstage（运维舞台）
...
- Python service can install the Python Agent（代理） package;
