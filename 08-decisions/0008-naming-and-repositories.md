...
## Repositories (CE v0.1)

| # | Repository | Edition | Language | Purpose |
|---|---|---|---|---|
| 1 | `xtrape-capsule-docs` | shared | n/a | Design docs, ADRs, OpenAPI / JSON spec SSOT (Layer 1 contracts) |
...
| 3 | `xtrape-capsule-agent-node` | shared | TypeScript | Node Agent SDK. Depends on `@xtrape/capsule-contracts-node`. |
| 4 | `xtrape-capsule-ce` | **CE-bound** | TypeScript | Opstage CE backend + UI + demo + deploy. pnpm workspace monorepo. |

### Future repositories (NOT in CE v0.1 scope, named here for forward-compatibility)
...
### Rule 1: Edition prefix appears ONLY on edition-bound repos

| Repo type | Edition prefix? |
|---|---|
| Backend / UI / deploy (control plane) | **Yes** — `xtrape-capsule-ce`, future `xtrape-capsule-opstage-ee`, `xtrape-capsule-opstage-cloud` |
| Agent SDK (per language) | **No** — `xtrape-capsule-agent-node`, never `-ce-agent-`, never `-ee-agent-` |
| Contracts bindings (per language) | **No** — `xtrape-capsule-contracts-node`, never `-ce-contracts-` |
| Documentation | **No** — `xtrape-capsule-docs` |

Reason: agents and contracts are wire-level; the same SDK and the same TS bindings MUST work against CE, EE, and Cloud backends without recompilation.

### Rule 2: Reverse-dependency prohibition

Edition-agnostic repos MUST NOT depend on edition-bound repos. Specifically:
...
CE consumes both `@xtrape/capsule-contracts-node` and `@xtrape/capsule-agent-node` from npm. Agent-node consumes `@xtrape/capsule-contracts-node` from npm. Nothing depends on CE.
...
## CE v0.1 Public npm Packages

Only two packages are published to public npm in CE v0.1:
...
CE-internal packages (inside `xtrape-capsule-ce` monorepo) are **NOT published** and exist only as workspace packages:
...
CE v0.1 publishes the following images to GHCR:
...
- `demo-capsule-service` is published as a separate image so users can `docker compose up` and immediately see a connected Capsule Service without cloning the repo.
- Future EE/Cloud will publish under `ghcr.io/xtrape/opstage-ee:*` / `ghcr.io/xtrape/opstage-cloud:*` (NOT in CE v0.1 scope).
...
Each repo versions independently; there is no synchronized "Opstage 0.1" meta-version. Compatibility is documented in each repo's README "Compatibility" section.
...
## Local Development: Multi-Repo Linking
...
- All four CE v0.1 repositories exist on GitHub under `xtrape/`.
...
- ADR 0009 — Contracts Spec and Bindings (defines Layer 1 / Layer 2 model and codegen rules for the contracts repos)
